...
bool ROCmFusionOpBatchNormActivation::IsFusionEligible(const Node* n2,
  ...
  if (isOpActivation(n2)) {  // activation node
    ...
    if (isOpBatchNorm(n1)) {  // preceded by a batchnorm node
      ...
      TF_CHECK_OK(GetNodeAttr(d->norm->def(), kAttr_is_training, &d->is_training));
      
      VLOG(kVlogLevel) << "===========";
      if (d->is_training) {
	DumpNodeList(kVlogLevel, "Found Fusion Candidate BatchNorm+Activation (training): ",
		     {d->norm, d->actv});
      }
      else {
	DumpNodeList(kVlogLevel, "Found Fusion Candidate BatchNorm+Activation (inference): ",
		     {d->norm, d->actv});
      }
  ...
  // Next check if the first output of batch-norm node feeds a node other then the activation node
  if (is_eligible) {
...
void ROCmFusionOpBatchNormActivation::CreateFusionOp(const FusionData* d) {
  ...
  // create an instance of the fusion node
  string op_name =
      strings::StrCat(d->norm->name(), d->actv->name());

  string fusion_type = d->is_training ? "_ROCmFusedBatchNormActivationTraining" : "_ROCmFusedBatchNormActivationInference";
  ...
  for (const Edge* e : d->norm->out_edges()) {
	...
	CHECK_NOTNULL(graph_->AddEdge(fusion_node, 1, e->dst(), e->dst_input()));
      }
      else if (e->src_output() == 1) { // batch-norm variance
	CHECK_NOTNULL(graph_->AddEdge(fusion_node, 2, e->dst(), e->dst_input()));
      }
    }
  }
  for (const Edge* e : d->actv->out_edges()) {
